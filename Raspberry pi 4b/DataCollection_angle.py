...
class BNO055:
	...
	# コンストラクタ
	##########
	def __init__(self, sensorId=-1, address=None):
		# インスタンス変数
...
def main():
    pot=MCP3008(0)
	# 変数定義、初期化
    bno1 = BNO055(address=0x28)
    bno2 = BNO055(address=0x29)
    data = np.empty((0,7))
    Datacollect = False
    print("Press 'SHIFT' to start/stop data collection and 'Ctrl+C' to exit.")
	
    if bno1.begin() is not True or bno2.begin() is not True:
        print("Error initializing device")
        exit()
    bno1.setExternalCrystalUse(True)
    bno2.setExternalCrystalUse(True)
    
    try:
        while True:
            x1, y1 , z1 = bno1.getVector(BNO055.VECTOR_EULER)
            x2,y2,z2 = bno2.getVector(BNO055.VECTOR_EULER)
            pot_value = f"{pot.value:.3f}"
            #print(f"{x1},{y1},{z1},{x2},{y2},{z2},{pot_value}")
            time.sleep(0.06)
            if Datacollect:
                feature= np.array([x1, y1, z1, x2, y2, z2, pot_value],dtype=float)
            ...
            if keyboard.is_pressed('shift'):
                ...
                if not Datacollect:
                    ...
                    print(f"Data saved to {filename}.scv")
