...
def main():
    ...
    while True:
        ...
        linear_accel1 = ','.join(map(str,linear_accel2))
        euler2 = ','.join(map(str,euler2))
        gyroscope2 = ','.join(map(str,gyroscope2))
        linear_accel2 = ','.join(map(str,linear_accel2))
        print("{}, {}, {}, {}, {}, {}, {}".format(euler1, gyroscope1, linear_accel1,euler2, gyroscope2, linear_accel2, pot.value))
        time.sleep(0.1)
        if pot.value < 1 and not timing:
            timing = True
        elif pot.value == 1 and timing:
            timing = False
            if len(data) > 0:
                original_indices = np.linspace(0, data.shape[0] - 1, num=data.shape[0])
                target_indices = np.linspace(0, data.shape[0] - 1, num=target_length)
                matrix = np.zeros((target_length, data.shape[1]))
                for i in range(data.shape[1]):
                    matrix[:, i] = np.interp(target_indices, original_indices, data[:, i])
                print(matrix.shape)
                #np.savetxt("data.csv", matrix, delimiter=",")
                data = np.empty((0, 18))
        if timing:
            feature= np.array([euler1, gyroscope1, linear_accel1, euler2, gyroscope2, linear_accel2])
